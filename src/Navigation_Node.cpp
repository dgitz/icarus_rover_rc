...
void ICARUS_SimRover_Pose_Callback(const geometry_msgs::Pose2D::ConstPtr& msg)
{
	try
	{
		...
		if(Rover_Status == STATUS_NAVIGATING)
		{
			if(distance_ref > 2.0)
			{
	...
	while( ros::ok() && INITIALIZED)
	{
		...
		else if(Operation_Mode == "SIM")
		{
			try
			{
				...
				Desired_Steering = Steer_Target + Steer_Obstacle;
				//printf("Steer: T: %f O: %f P: %f\r\n",Steer_Target,Steer_Obstacle,Desired_Steering);
				...
				if((abs(Desired_Throttle) < 0.05) and (Rover_Status == STATUS_NAVIGATING))
				{
					//printf("I'm stuck!\r\n");
					stuck_timer++;
				}
				if((abs(Desired_Throttle) > 0.5) and (Rover_Status == STATUS_NAVIGATING))
				{
					stuck_timer = 0;
				}
				double temp = Forward_Neutral_Reverse * stuck_timer * .05;
				Desired_Throttle = Desired_Throttle + temp;
				//printf("Throttle: T: %f O: %f P: %f\r\n",Gene_Throttle_Target,Throttle_Obstacle,Desired_Throttle);
